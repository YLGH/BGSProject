...
*******************************************************************************/
    ...
    *  callbackNum: The number of the callback function address to be set.
    ...
    *
    *******************************************************************************/
    cyWdtCallback CySysWdtSetIsrCallback(uint32 counterNum, cyWdtCallback function)
    {
    ...
    *
    *******************************************************************************/
    cyWdtCallback CySysWdtGetIsrCallback(uint32 counterNum)
    {
    ...
    *******************************************************************************/
    void CySysWdtIsr(void)
    {
        ...
        {
            interruptMask |= CY_SYS_WDT_COUNTER0_INT;
        ...
        {
            interruptMask |= CY_SYS_WDT_COUNTER1_INT;
        ...
        {
            interruptMask |= CY_SYS_WDT_COUNTER2_INT;
        ...
        CySysWdtClearInterrupt(interruptMask);
    }
    ...
    *
    * Summary:
    *  Adds a user-specified delay in the LFCLK clock ticks. This function can operate
    *  in two modes: the "WAIT" mode and "INTERRUPT" mode.
    *
    *  In the "WAIT" mode, the function waits until the WDT counter reaches the
    *  requested number of the delay ticks above the value when the function is entered.
    *
    *  In the "INTERRUPT" mode, this function reads the current counter value, adds
    *  an offset and stores back to the match. When the timer expires, the
    *  interrupt is triggered.
    *
    * Parameters:
    *  counterNum: The number of the WDT counter (WDT0 or WDT1).
    ...
    *
    *  Side effects:
    *   In the "WAIT" mode the code execution gets stuck in this function if
    *   an appropriate WDT is stopped.
    *
    *   In the "WAIT" mode the function has no effect if an appropriate
    *   WDT is not configured as "Free Running Timer". "Clear on match"
    *   should be disabled.
    *
    *******************************************************************************/
    ...
    *
    * Summary:
    *  Provides a user-specified delay till the counter value matches the "match"
    *  value for the "Free Running Timer" mode. The delay can be the "WAIT" type or
    *  "INTERRUPT" type.
    *
    * Parameters:
    *  counterNum: The number of the WDT counter.
    *  delayType:  Defines the operation mode.
    *               CY_SYS_TIMER_WAIT       0
    *               CY_SYS_TIMER_INTERRUPT  1
    *
    *  match:      The match value (timestamp) in the LFCLK ticks (allowable range - 16-bit
    *              value).
    *
    * Return:
    *  None.
    ...
    *  an appropriate WDT is stopped.
    *
    *   In the "WAIT" mode function has no effect if the appropriate
    *   WDT is not configured as the "Free Running Timer". "Clear on match"
    *   should be disabled.
    *
    *******************************************************************************/
    ...
    *
    *******************************************************************************/
    cyWdtCallback CySysWdtSetIsrCallback(cyWdtCallback function)
    {
        cyWdtCallback prevCallback = (void *)0;
    ...
    *
    * Parameters:
    *  callbackNum: The number of callback function address to be set.
    *               The valid range is from 0 to 2.
    *
    * Return:
    ...
    *
    *******************************************************************************/
    cyWdtCallback CySysWdtGetIsrCallback(void)
    {   
