...
boolean SDClass::begin() {
   ...
   */
    uint8_t cardgood = card.init(SPI_HALF_SPEED);
    if(!cardgood) return false;
    uint8_t volgood = volume.init(card);
    if(!volgood) return false;
    uint8_t rootgood = root.openRoot(volume);
    if(!rootgood && !root.isOpen()) return false;
    return true;
