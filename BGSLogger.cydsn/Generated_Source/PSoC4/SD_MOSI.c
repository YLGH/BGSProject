...
#define SetP4PinDriveMode(shift, mode)  \
    do { \
        SD_MOSI_PC =   (SD_MOSI_PC & \
                                (uint32)(~(uint32)(SD_MOSI_DRIVE_MODE_IND_MASK << (SD_MOSI_DRIVE_MODE_BITS * (shift))))) | \
                                (uint32)((uint32)(mode) << (SD_MOSI_DRIVE_MODE_BITS * (shift))); \
    } while (0)
...
{
    uint8 drVal = (uint8)(SD_MOSI_DR & (uint8)(~SD_MOSI_MASK));
    drVal = (drVal | ((uint8)(value << SD_MOSI_SHIFT) & SD_MOSI_MASK));
    SD_MOSI_DR = (uint32)drVal;
}
...
{
	SetP4PinDriveMode(SD_MOSI__0__SHIFT, mode);
}
...
{
    return (uint8)((SD_MOSI_PS & SD_MOSI_MASK) >> SD_MOSI_SHIFT);
}
...
{
    return (uint8)((SD_MOSI_DR & SD_MOSI_MASK) >> SD_MOSI_SHIFT);
}
...
/* If Interrupts Are Enabled for this Pins component */ 
    ...
    *******************************************************************************/
    ...
    {
		uint8 maskedStatus = (uint8)(SD_MOSI_INTSTAT & SD_MOSI_MASK);
		SD_MOSI_INTSTAT = maskedStatus;
        return maskedStatus >> SD_MOSI_SHIFT;
    }
