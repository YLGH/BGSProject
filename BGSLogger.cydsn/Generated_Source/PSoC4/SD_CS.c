...
#define SetP4PinDriveMode(shift, mode)  \
    do { \
        SD_CS_PC =   (SD_CS_PC & \
                                (uint32)(~(uint32)(SD_CS_DRIVE_MODE_IND_MASK << (SD_CS_DRIVE_MODE_BITS * (shift))))) | \
                                (uint32)((uint32)(mode) << (SD_CS_DRIVE_MODE_BITS * (shift))); \
    } while (0)
...
{
	SetP4PinDriveMode(SD_CS__0__SHIFT, mode);
}
