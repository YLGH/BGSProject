...
#define SetP4PinDriveMode(shift, mode)  \
    do { \
        SD_MISO_PC =   (SD_MISO_PC & \
                                (uint32)(~(uint32)(SD_MISO_DRIVE_MODE_IND_MASK << (SD_MISO_DRIVE_MODE_BITS * (shift))))) | \
                                (uint32)((uint32)(mode) << (SD_MISO_DRIVE_MODE_BITS * (shift))); \
    } while (0)
...
{
    uint8 drVal = (uint8)(SD_MISO_DR & (uint8)(~SD_MISO_MASK));
    drVal = (drVal | ((uint8)(value << SD_MISO_SHIFT) & SD_MISO_MASK));
    SD_MISO_DR = (uint32)drVal;
}
...
{
	SetP4PinDriveMode(SD_MISO__0__SHIFT, mode);
}
...
{
    return (uint8)((SD_MISO_PS & SD_MISO_MASK) >> SD_MISO_SHIFT);
}
...
{
    return (uint8)((SD_MISO_DR & SD_MISO_MASK) >> SD_MISO_SHIFT);
}
...
/* If Interrupts Are Enabled for this Pins component */ 
    ...
    *******************************************************************************/
    ...
    {
		uint8 maskedStatus = (uint8)(SD_MISO_INTSTAT & SD_MISO_MASK);
		SD_MISO_INTSTAT = maskedStatus;
        return maskedStatus >> SD_MISO_SHIFT;
    }
