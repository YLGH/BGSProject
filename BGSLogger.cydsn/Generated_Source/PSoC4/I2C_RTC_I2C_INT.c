...
CY_ISR(I2C_RTC_I2C_ISR)
{
    ...
    if(NULL != I2C_RTC_customIntrHandler)
    {
        I2C_RTC_customIntrHandler();
    }

    if(I2C_RTC_CHECK_INTR_I2C_EC_MASKED(I2C_RTC_INTR_I2C_EC_WAKE_UP))
    {
        /* Mask-off after wakeup */
        I2C_RTC_SetI2CExtClkInterruptMode(I2C_RTC_NO_INTR_SOURCES);
    }
    ...
    */
    if(I2C_RTC_CHECK_I2C_FSM_MASTER)
    {
        #if(I2C_RTC_I2C_MASTER)
        {
            ...
            */
            if(I2C_RTC_CHECK_INTR_MASTER_MASKED(I2C_RTC_INTR_MASTER_I2C_BUS_ERROR))
            {
                I2C_RTC_mstrStatus |= (uint16) (I2C_RTC_I2C_MSTAT_ERR_XFER |
                                                         I2C_RTC_I2C_MSTAT_ERR_BUS_ERROR);

                endTransfer = I2C_RTC_I2C_CMPLT_ANY_TRANSFER;
            }
            ...
            */
            if(I2C_RTC_CHECK_INTR_MASTER_MASKED(I2C_RTC_INTR_MASTER_I2C_ARB_LOST))
            {
                I2C_RTC_mstrStatus |= (uint16) (I2C_RTC_I2C_MSTAT_ERR_XFER |
                                                         I2C_RTC_I2C_MSTAT_ERR_ARB_LOST);

                endTransfer = I2C_RTC_I2C_CMPLT_ANY_TRANSFER;
            }

            #if(I2C_RTC_I2C_MULTI_MASTER_SLAVE)
            {
                /* I2C_MASTER_CMD_M_START_ON_IDLE:
                * MultiMaster-Slave does not generate start, because Slave was addressed.
                * Pass control to slave.
                */
                if(I2C_RTC_CHECK_I2C_MASTER_CMD(I2C_RTC_I2C_MASTER_CMD_M_START_ON_IDLE))
                {
                    I2C_RTC_mstrStatus |= (uint16) (I2C_RTC_I2C_MSTAT_ERR_XFER |
                                                             I2C_RTC_I2C_MSTAT_ERR_ABORT_XFER);
            ...
            */
            if(0u != endTransfer)
            {
                ...
                I2C_RTC_mstrStatus |= (uint16) I2C_RTC_GET_I2C_MSTAT_CMPLT;

                #if(I2C_RTC_I2C_MULTI_MASTER_SLAVE)
                {
                    if(I2C_RTC_CHECK_I2C_FSM_ADDR)
                    {
                        ...
                        */
                        if(I2C_RTC_CHECK_I2C_MASTER_CMD(I2C_RTC_I2C_MASTER_CMD_M_START_ON_IDLE))
                        {
                            I2C_RTC_I2C_MASTER_CLEAR_START;

                            endTransfer = I2C_RTC_I2C_CMPLT_ANY_TRANSFER; /* Pass control to Slave */
                        }
                        ...
                        */
                        else if((!I2C_RTC_CHECK_INTR_SLAVE_MASKED(I2C_RTC_INTR_SLAVE_I2C_BUS_ERROR))
                               && I2C_RTC_CHECK_INTR_MASTER_MASKED(I2C_RTC_INTR_MASTER_I2C_ARB_LOST))
                        {
                            endTransfer = I2C_RTC_I2C_CMPLT_ANY_TRANSFER; /* Pass control to Slave */
                        }
                        else
                        ...
                        {
                            I2C_RTC_CLEAR_TX_FIFO; /* Shifter keeps address, clear it */

                            I2C_RTC_DISABLE_MASTER_AUTO_DATA_ACK; /* In case of reading disable autoACK */

                            /* Clean-up master interrupt sources */
                            I2C_RTC_ClearMasterInterruptSource(I2C_RTC_INTR_MASTER_ALL);

                            /* Disable data processing interrupts: they have to be cleared before */
                            I2C_RTC_SetRxInterruptMode(I2C_RTC_NO_INTR_SOURCES);
                            I2C_RTC_SetTxInterruptMode(I2C_RTC_NO_INTR_SOURCES);

                            I2C_RTC_state = I2C_RTC_I2C_FSM_IDLE;
                        }
                        else
                        {
                            /* Set I2C_FSM_EXIT_IDLE for BUS_ERR and ARB_LOST (that is really bus error) */
                            I2C_RTC_state = I2C_RTC_I2C_FSM_EXIT_IDLE;
                        }
                    }
                    else
                    {
    ...
    else /* (I2C_RTC_CHECK_I2C_FSM_SLAVE) */
    {
        #if(I2C_RTC_I2C_SLAVE)
        {
            /* INTR_SLAVE_I2C_BUS_ERROR or I2C_RTC_INTR_SLAVE_I2C_ARB_LOST:
            * A Misplaced Start or Stop condition occurred on the bus: set a flag
            * to notify an error condition.
            */
            if(I2C_RTC_CHECK_INTR_SLAVE_MASKED(I2C_RTC_INTR_SLAVE_I2C_BUS_ERROR |
                                                        I2C_RTC_INTR_SLAVE_I2C_ARB_LOST))
            {
                if(I2C_RTC_CHECK_I2C_FSM_RD)
                {
                    /* TX direction: master reads from slave */
                    I2C_RTC_slStatus &= (uint8) ~I2C_RTC_I2C_SSTAT_RD_BUSY;
                    I2C_RTC_slStatus |= (uint8) (I2C_RTC_I2C_SSTAT_RD_ERR |
                                                          I2C_RTC_I2C_SSTAT_RD_CMPLT);
                }
                else
                {
                    /* RX direction: master writes into slave */
                    I2C_RTC_slStatus &= (uint8) ~I2C_RTC_I2C_SSTAT_WR_BUSY;
                    I2C_RTC_slStatus |= (uint8) (I2C_RTC_I2C_SSTAT_WR_ERR |
                                                          I2C_RTC_I2C_SSTAT_WR_CMPLT);
                }
    ...
    */
    ...
    if(I2C_RTC_CHECK_I2C_FSM_MASTER)
    {
        #if(I2C_RTC_I2C_MASTER)
        {
            ...
            */
            if(I2C_RTC_CHECK_INTR_MASTER_MASKED(I2C_RTC_INTR_MASTER_I2C_STOP))
            {
                I2C_RTC_ClearMasterInterruptSource(I2C_RTC_INTR_MASTER_I2C_STOP);
            ...
            {
                if(I2C_RTC_CHECK_I2C_FSM_ADDR) /* Address stage */
                {
                    /* INTR_MASTER_I2C_NACK:
                    * The master sent an address but it was NACKed by the slave. Complete transaction.
                    */
                    if(I2C_RTC_CHECK_INTR_MASTER_MASKED(I2C_RTC_INTR_MASTER_I2C_NACK))
                    {
                        I2C_RTC_ClearMasterInterruptSource(I2C_RTC_INTR_MASTER_I2C_NACK);

                        I2C_RTC_mstrStatus |= (uint16) (I2C_RTC_I2C_MSTAT_ERR_XFER |
                                                                 I2C_RTC_I2C_MSTAT_ERR_ADDR_NAK);
                    ...
                    */
                    ...
                    {
                        if(I2C_RTC_CHECK_I2C_FSM_RD) /* Reading */
                        {
                            I2C_RTC_state = I2C_RTC_I2C_FSM_MSTR_RD_DATA;
                        }
                        else /* Writing */
                        {
                            I2C_RTC_state = I2C_RTC_I2C_FSM_MSTR_WR_DATA;
                            if(0u != I2C_RTC_mstrWrBufSize)
                            {
                                /* Enable INTR.TX_EMPTY if there is data to transmit */
                                I2C_RTC_SetTxInterruptMode(I2C_RTC_INTR_TX_EMPTY);
                            }
                        }
                    }
                }

                if(I2C_RTC_CHECK_I2C_FSM_DATA) /* Data phase */
                {
                    if(I2C_RTC_CHECK_I2C_FSM_RD) /* Reading */
                    {
                        /* INTR_RX_FULL:
                        ...
                        */
                        if(I2C_RTC_CHECK_INTR_RX_MASKED(I2C_RTC_INTR_RX_FULL))
                        {
                            /* Calculate difference */
                            diffCount =  I2C_RTC_mstrRdBufSize -
                                        (I2C_RTC_mstrRdBufIndex + I2C_RTC_GET_RX_FIFO_ENTRIES);

                            /* Proceed transaction or end it when RX FIFO becomes FULL again */
                            if(diffCount > I2C_RTC_I2C_FIFO_SIZE)
                            {
                                diffCount = I2C_RTC_I2C_FIFO_SIZE;
                            }
                            else
                            {
                                if(0u == diffCount)
                                {
                                    I2C_RTC_DISABLE_MASTER_AUTO_DATA_ACK;
                            ...
                            {
                                I2C_RTC_mstrRdBufPtr[I2C_RTC_mstrRdBufIndex] = (uint8)
                                                                                        I2C_RTC_RX_FIFO_RD_REG;
                        ...
                        */
                        else if(I2C_RTC_CHECK_INTR_RX_MASKED(I2C_RTC_INTR_RX_NOT_EMPTY))
                        {
                            /* Put data in component buffer */
                            I2C_RTC_mstrRdBufPtr[I2C_RTC_mstrRdBufIndex] = (uint8) I2C_RTC_RX_FIFO_RD_REG;
                            I2C_RTC_mstrRdBufIndex++;

                            if(I2C_RTC_mstrRdBufIndex < I2C_RTC_mstrRdBufSize)
                            {
                                I2C_RTC_I2C_MASTER_GENERATE_ACK;
                            }
                            else
                            {
                               endTransfer = I2C_RTC_I2C_CMPLT_ANY_TRANSFER;
                        ...
                        I2C_RTC_ClearRxInterruptSource(I2C_RTC_INTR_RX_ALL);
                    }
                    else /* Writing */
                    {
                        /* INTR_MASTER_I2C_NACK :
                        ...
                        */
                        if(I2C_RTC_CHECK_INTR_MASTER_MASKED(I2C_RTC_INTR_MASTER_I2C_NACK))
                        {
                            ...
                            I2C_RTC_mstrWrBufIndexTmp -= (I2C_RTC_GET_TX_FIFO_ENTRIES +
                                                                   I2C_RTC_GET_TX_FIFO_SR_VALID);

                            /* Update number of transferred bytes */
                            I2C_RTC_mstrWrBufIndex = I2C_RTC_mstrWrBufIndexTmp;

                            I2C_RTC_mstrStatus |= (uint16) (I2C_RTC_I2C_MSTAT_ERR_XFER |
                                                                     I2C_RTC_I2C_MSTAT_ERR_SHORT_XFER);

                            I2C_RTC_CLEAR_TX_FIFO;
                        ...
                        */
                        else if(I2C_RTC_CHECK_INTR_TX_MASKED(I2C_RTC_INTR_TX_EMPTY))
                        {
                            while(I2C_RTC_I2C_FIFO_SIZE != I2C_RTC_GET_TX_FIFO_ENTRIES)
                            {
                                ...
                                */
                                if(I2C_RTC_mstrWrBufIndexTmp < I2C_RTC_mstrWrBufSize)
                                {
                                    ...
                                    */
                                    if(I2C_RTC_mstrWrBufIndexTmp == (I2C_RTC_mstrWrBufSize - 1u))
                                    {
                                        I2C_RTC_ClearTxInterruptSource(I2C_RTC_INTR_TX_UNDERFLOW);
                                        I2C_RTC_SetTxInterruptMode(I2C_RTC_INTR_TX_UNDERFLOW);
                                    }
                                 #endif /* (!I2C_RTC_CY_SCBIP_V0) */

                                    /* Put data into TX FIFO */
                                    I2C_RTC_TX_FIFO_WR_REG = (uint32) I2C_RTC_mstrWrBufPtr[I2C_RTC_mstrWrBufIndexTmp];
                                    I2C_RTC_mstrWrBufIndexTmp++;
                                }
                                else
                                {
                            ...
                            I2C_RTC_ClearTxInterruptSource(I2C_RTC_INTR_TX_ALL);
                        #else
                            I2C_RTC_ClearTxInterruptSource(I2C_RTC_INTR_TX_EMPTY);
                        #endif /* (I2C_RTC_CY_SCBIP_V0) */
                        }
                        /* INTR_TX_UNDERFLOW:
                        ...
                        */
                        else if(I2C_RTC_CHECK_INTR_TX_MASKED(I2C_RTC_INTR_TX_UNDERFLOW))
                        {
                            /* Update number of transferred bytes */
                            I2C_RTC_mstrWrBufIndex = I2C_RTC_mstrWrBufIndexTmp;
                ...
                if(0u != endTransfer) /* Complete transfer */
                {
                    ...
                    if(I2C_RTC_CHECK_I2C_MODE_NO_STOP(I2C_RTC_mstrControl))
                    {
                        /* On-going transaction is suspended: the ReStart is generated by the API request */
                        I2C_RTC_mstrStatus |= (uint16) (I2C_RTC_I2C_MSTAT_XFER_HALT |
                                                                 I2C_RTC_GET_I2C_MSTAT_CMPLT);

                        I2C_RTC_state = I2C_RTC_I2C_FSM_MSTR_HALT;
                    }
                    else
                    {
                        ...
                        */
                        I2C_RTC_I2C_MASTER_GENERATE_STOP;
                    }
                }
            }
    ...
    } /* (I2C_RTC_CHECK_I2C_FSM_MASTER) */


    /* FSM Slave */
    else if(I2C_RTC_CHECK_I2C_FSM_SLAVE)
    {
        #if(I2C_RTC_I2C_SLAVE)
        {
            /* INTR_SLAVE_NACK:
            ...
            */
            if(I2C_RTC_CHECK_INTR_SLAVE_MASKED(I2C_RTC_INTR_SLAVE_I2C_NACK))
            {
                I2C_RTC_ClearSlaveInterruptSource(I2C_RTC_INTR_SLAVE_I2C_NACK);

                /* All entries that remain in TX FIFO max value is 9: 8 (FIFO) + 1 (SHIFTER) */
                diffCount = (I2C_RTC_GET_TX_FIFO_ENTRIES + I2C_RTC_GET_TX_FIFO_SR_VALID);

                if(I2C_RTC_slOverFlowCount > diffCount) /* Overflow */
                {
                    I2C_RTC_slStatus |= (uint8) I2C_RTC_I2C_SSTAT_RD_OVFL;
                }
                else /* No Overflow */
                {
                    /* Roll-back temporary index */
                    I2C_RTC_slRdBufIndexTmp -= (diffCount - I2C_RTC_slOverFlowCount);
                }

                /* Update slave of transferred bytes */
                I2C_RTC_slRdBufIndex = I2C_RTC_slRdBufIndexTmp;

                /* Clean-up TX FIFO */
                I2C_RTC_SetTxInterruptMode(I2C_RTC_NO_INTR_SOURCES);
                I2C_RTC_slOverFlowCount = 0u;
                I2C_RTC_CLEAR_TX_FIFO;

                /* Complete master reading */
                I2C_RTC_slStatus &= (uint8) ~I2C_RTC_I2C_SSTAT_RD_BUSY;
                I2C_RTC_slStatus |= (uint8)  I2C_RTC_I2C_SSTAT_RD_CMPLT;
                I2C_RTC_state     =  I2C_RTC_I2C_FSM_IDLE;
            }
            ...
            */
            if(I2C_RTC_CHECK_INTR_SLAVE_MASKED(I2C_RTC_INTR_SLAVE_I2C_WRITE_STOP))
            {
                I2C_RTC_ClearSlaveInterruptSource(I2C_RTC_INTR_SLAVE_I2C_WRITE_STOP);
                ...
                * I2C_ADDR_MATCH.
                */
                if (0u != (I2C_RTC_I2C_CTRL_REG & I2C_RTC_I2C_CTRL_S_READY_DATA_ACK))
                {
                    while(0u != I2C_RTC_GET_RX_FIFO_ENTRIES)
                    {
                        #if(I2C_RTC_CHECK_I2C_ACCEPT_ADDRESS)
                        {
                            if((1u == I2C_RTC_GET_RX_FIFO_ENTRIES) &&
                               (I2C_RTC_CHECK_INTR_SLAVE_MASKED(I2C_RTC_INTR_SLAVE_I2C_ADDR_MATCH)))
                            {
                        ...
                        /* Put data in component buffer */
                        I2C_RTC_slWrBufPtr[I2C_RTC_slWrBufIndex] = (uint8) I2C_RTC_RX_FIFO_RD_REG;
                        I2C_RTC_slWrBufIndex++;
                    }

                    I2C_RTC_DISABLE_SLAVE_AUTO_DATA;
                }

                if(I2C_RTC_CHECK_INTR_RX(I2C_RTC_INTR_RX_OVERFLOW))
                {
                    I2C_RTC_slStatus |= (uint8) I2C_RTC_I2C_SSTAT_WR_OVFL;
                }
                ...
                I2C_RTC_slStatus &= (uint8) ~I2C_RTC_I2C_SSTAT_WR_BUSY;
                I2C_RTC_slStatus |= (uint8)  I2C_RTC_I2C_SSTAT_WR_CMPLT;
                I2C_RTC_state     =  I2C_RTC_I2C_FSM_IDLE;
            }
            ...
            */
        #if (I2C_RTC_I2C_CUSTOM_ADDRESS_HANDLER_CONST)
            if (I2C_RTC_CHECK_INTR_SLAVE_MASKED(I2C_RTC_INTR_SLAVE_I2C_ADDR_MATCH |
        ...
        #endif /* (I2C_RTC_I2C_CUSTOM_ADDRESS_HANDLER_CONST) */
            {
                ...
                I2C_RTC_ClearI2CExtClkInterruptSource(I2C_RTC_INTR_I2C_EC_WAKE_UP);

                #if (I2C_RTC_I2C_CUSTOM_ADDRESS_HANDLER)
                {
                    if (NULL != I2C_RTC_customAddressHandler)
                    {
                        /* Call custom address handler */
                        response = I2C_RTC_customAddressHandler();
                    }
                    else
                    {
                        /* Read address from the RX FIFO. If there is no address underflow triggers but
                        * componnet does not use that source. */
                        (void) I2C_RTC_RX_FIFO_RD_REG;
                        response = I2C_RTC_I2C_ACK_ADDR;
                    }
            ...
            #if (I2C_RTC_I2C_CUSTOM_ADDRESS_HANDLER_CONST)
                if (response == I2C_RTC_I2C_NAK_ADDR)
                {
                #if (!I2C_RTC_CY_SCBIP_V0)
                    /* Disable write stop interrupt source as it triggers after address was NACKed. Ticket ID#156094 */
                    I2C_RTC_DISABLE_INTR_SLAVE(I2C_RTC_INTR_SLAVE_I2C_WRITE_STOP);
                #endif /* (!I2C_RTC_CY_SCBIP_V0) */

                    /* Clear address match and stop history */
                    I2C_RTC_ClearSlaveInterruptSource(I2C_RTC_INTR_SLAVE_ALL);

                    /* ACK the address byte */
                    I2C_RTC_I2C_SLAVE_GENERATE_NACK;
                }
                else
            #endif /* (I2C_RTC_I2C_CUSTOM_ADDRESS_HANDLER_CONST) */
                {
                    if(I2C_RTC_CHECK_I2C_STATUS(I2C_RTC_I2C_STATUS_S_READ))
                    /* TX direction: master reads from slave */
                    {
                        I2C_RTC_SetTxInterruptMode(I2C_RTC_INTR_TX_EMPTY);

                        /* Set temporary index to address buffer clear from API */
                        I2C_RTC_slRdBufIndexTmp = I2C_RTC_slRdBufIndex;

                        /* Start master reading */
                        I2C_RTC_slStatus |= (uint8) I2C_RTC_I2C_SSTAT_RD_BUSY;
                        I2C_RTC_state     = I2C_RTC_I2C_FSM_SL_RD;
                    }
                    else
                    /* RX direction: master writes into slave */
                    {
                        /* Calculate available buffer size */
                        diffCount = (I2C_RTC_slWrBufSize - I2C_RTC_slWrBufIndex);

                    #if (I2C_RTC_CY_SCBIP_V0)
                        if(diffCount < I2C_RTC_I2C_FIFO_SIZE)
                        /* Receive data: byte-by-byte */
                        {
                            I2C_RTC_SetRxInterruptMode(I2C_RTC_INTR_RX_NOT_EMPTY);
                        }
                        else
                        /* Receive data: into RX FIFO */
                        {
                            if(diffCount == I2C_RTC_I2C_FIFO_SIZE)
                            {
                                ...
                                I2C_RTC_ENABLE_SLAVE_AUTO_DATA;
                            }
                            else
                            {
                                /* Stretch clock when RX FIFO becomes FULL */
                                I2C_RTC_ENABLE_SLAVE_AUTO_DATA_ACK;
                                I2C_RTC_SetRxInterruptMode(I2C_RTC_INTR_RX_FULL);
                            }
                        }
                        ...
                        /* Start master reading */
                        I2C_RTC_slStatus |= (uint8) I2C_RTC_I2C_SSTAT_WR_BUSY;
                        I2C_RTC_state     = I2C_RTC_I2C_FSM_SL_WR;
                    }
                    ...
                    I2C_RTC_ENABLE_INTR_SLAVE(I2C_RTC_INTR_SLAVE_I2C_WRITE_STOP);
                #endif /* (!I2C_RTC_CY_SCBIP_V0) */

                    /* ACK the address byte */
                    I2C_RTC_I2C_SLAVE_GENERATE_ACK;
                }
            }
            ...
            */
            if(I2C_RTC_CHECK_INTR_RX_MASKED(I2C_RTC_INTR_RX_FULL))
            {
                ...
                diffCount =  I2C_RTC_slWrBufSize -
                            (I2C_RTC_slWrBufIndex + I2C_RTC_I2C_FIFO_SIZE);

                if(diffCount > I2C_RTC_I2C_FIFO_SIZE) /* Proceed transaction */
                {
                    diffCount   = I2C_RTC_I2C_FIFO_SIZE;
                    endTransfer = 0u;  /* Continue active transfer */
                }
                ...
                {
                    I2C_RTC_ENABLE_SLAVE_AUTO_DATA_NACK;
                ...
                I2C_RTC_ClearRxInterruptSource(I2C_RTC_INTR_RX_FULL);
            }
            ...
            */
            ...
            {
                diffCount = I2C_RTC_RX_FIFO_RD_REG;
                ...
                if(I2C_RTC_slWrBufIndex < I2C_RTC_slWrBufSize)
                {
                    ...
                    I2C_RTC_slWrBufPtr[I2C_RTC_slWrBufIndex] = (uint8) diffCount;
                    I2C_RTC_slWrBufIndex++;
                }
                ...
                I2C_RTC_ClearRxInterruptSource(I2C_RTC_INTR_RX_NOT_EMPTY);
            }
            else
            {
                /* Does nothing */
            ...
            */
            if(I2C_RTC_CHECK_INTR_TX_MASKED(I2C_RTC_INTR_TX_EMPTY))
            {
                while(I2C_RTC_I2C_FIFO_SIZE != I2C_RTC_GET_TX_FIFO_ENTRIES)
                {
                    ...
                    */
                    if(I2C_RTC_slRdBufIndexTmp < I2C_RTC_slRdBufSize)
                    /* Data from buffer */
                    {
                        I2C_RTC_TX_FIFO_WR_REG = (uint32) I2C_RTC_slRdBufPtr[I2C_RTC_slRdBufIndexTmp];
                        I2C_RTC_slRdBufIndexTmp++;
                    }
                    else
                    /* Probably Overflow */
                    {
                        I2C_RTC_TX_FIFO_WR_REG = I2C_RTC_I2C_SLAVE_OVFL_RETURN;

                        if(0u == (I2C_RTC_INTR_TX_OVERFLOW & I2C_RTC_slOverFlowCount))
                        {
                            /* Get counter in range of byte: value 10 is overflow */
                            I2C_RTC_slOverFlowCount++;
                        }
                    }
                }
    ...
    */
    ...
    {
        I2C_RTC_CTRL_REG &= (uint32) ~I2C_RTC_CTRL_ENABLED; /* Disable scb IP */

        I2C_RTC_state = I2C_RTC_I2C_FSM_IDLE;

        I2C_RTC_DISABLE_SLAVE_AUTO_DATA;
        I2C_RTC_DISABLE_MASTER_AUTO_DATA;
        ...
        I2C_RTC_ClearTxInterruptSource(I2C_RTC_INTR_RX_ALL);
        I2C_RTC_ClearRxInterruptSource(I2C_RTC_INTR_TX_ALL);
        I2C_RTC_ClearSlaveInterruptSource(I2C_RTC_INTR_SLAVE_ALL);
        I2C_RTC_ClearMasterInterruptSource(I2C_RTC_INTR_MASTER_ALL);
    #endif /* (I2C_RTC_CY_SCBIP_V0) */

        I2C_RTC_CTRL_REG |= (uint32) I2C_RTC_CTRL_ENABLED;  /* Enable scb IP */
    }
}
