...
class SensorPlot:
	...
	def add_sensor(self, index, graphTitle = "New Sensor"):
		index -= 1;
		assert(not sensors[index].toLog), "This sensor is already added!"
		...
		self.curve[index] = self.p1.plot(pen = 'r')
	...
	def update(self):
		for i in range(0, 4):
			if(self.sensor[0].toLog):
				self.curve[index].setData(self.qx, self.data[0])
